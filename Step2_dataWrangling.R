...
#note: the $ sign will give you a pull-down of the different columns in the columns within a dataframe or the elements within a list
myTPM <- Txi_gene$abundance
#note:if you go to the Environment section (top right hand corner), and click on the myTPM object you just made, 
#it will open up the table, in which the row names are the 
...
targets
#since my data has diff naming system, instead of running the following code
#sampleLabels <- targets$sample
#I will instead run the following code:
sampleLabels <- targets$sra_accession
#print sampleLabels to see what it looks like
sampleLabels
...
log2.cpm.df.pivot <- pivot_longer(log2.cpm.df, # dataframe to be pivoted
                                  cols = 2:11, # column names to be stored as a SINGLE variable
